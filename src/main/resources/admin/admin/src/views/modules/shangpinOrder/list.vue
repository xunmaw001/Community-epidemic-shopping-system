                ...
                <el-row class="ad" :style="{justifyContent:contents.btnAdAllBoxPosition=='1'?'flex-start':contents.btnAdAllBoxPosition=='2'?'center':'flex-end'}">
                    <el-form-item>
                        ...
                        <a style="text-decoration:none" class="el-button el-button--success"
                           ...
                           href="http://localhost:8080/xiaoquyiqingguanlixitong/upload/shangpinOrderMuBan.xls"
                        >批量导入商品订单数据模板</a>
                        &nbsp;
                        <el-upload
                                v-if="isAuth('shangpinOrder','导入导出')"
...
&& sessionTable=='shangjia' && userId==scope.row.shangpinShangjiaId" type="primary" icon="el-icon-reading" size="mini"
                            ...
                            >已备好</el-button>
                          <el-button v-if="sessionTable=='peisongrenyuan' && scope.row.shangpinOrderTypes == 103 && scope.row.peisongrenyuanId==null" type="primary" icon="el-icon-reading" size="mini"
                               @click="peisong(scope.row.id)"
                            >已配送</el-button>
...
<script>
    ...
    //当前表下拉框搜索条件
        ...
        peisong(id){
				let _this = this;
				_this.$confirm(`确定已配送吗?`, "提示", {
					confirmButtonText: "确定",
					cancelButtonText: "取消",
					type: "warning"
				}).then(() => {
					_this.$http({
						url: "shangpinOrder/peisong?id="+id,
						method: "post",
					}).then(({ data }) => {
						if (data && data.code === 0) {
							_this.$message({
